    ...
    <div class="app-content ">
        <div class="app-content-body">
            <div class="bg-light lter b-b wrapper-md ">
                <h1 class="m-n font-thin h3">公众号管理>文件批量上传</h1>
            </div>
            ...
            <div class="wrapper-md">
                <form class="form-horizontal" id="templateForm"
                      action="<%=request.getContextPath()%>/admin/publicNumber/template/download" method="post">
                    <div class="col-xs-24 col-md-12 b  line-dashed wrapper-sm padder">
                        <div class="form-group row">
                            ...
                            <input id="qrCodes" type="hidden" name="qrCodes"/>
                        </div>
                    </div>
                </form>
            </div>
...
<script type="text/javascript">
    ...
    function templateUpload() {
        ...
        if (files && files[0]) {
            ...
            uploadFile('<%=request.getContextPath()%>/admin/publicNumber/template/upload', formData, function (data) {
                if (!data.status) {
                    $("#info").html(data.msg);
    ...
    function uploadPreview(file) {
        if (file.files) {
            ...
            for (var i = 0; i < file.files.length; ++i) {
                client.multipartUpload("app/" + Date.parse(new Date()) + "_" + file.files[i].name, file.files[i], {
                    progress: function*(p) {
