...
<div class="app app-header-fixed app-aside-fixed">
    ...
    <%@include file="./publicNumber_nav.jsp" %>
    <div class="app-content ">
        <div class="app-content-body">
            <div class="bg-light lter b-b wrapper-md ">
                <h1 class="m-n font-thin h3">公众号管理</h1>
            </div>
            <div class="wrapper-md">
                <form class="form-horizontal" id="searchForm">
                    ...
                    <div class="form-group m-t-n-md">
                        <div class="col-xs-12">
                            <sec:authorize access="hasAnyRole(${sessionScope.sec_op.publicNumber_publicNumber_save})">
                                <button class="btn btn-success" type="button" data-toggle="modal"
                                        data-target="#savePublicNumber">
                                    新增
                                </button>
                            </sec:authorize>
                            <sec:authorize access="hasAnyRole(${sessionScope.sec_op.publicNumber_publicNumber_batchUpload})">
                            <a href="<%=request.getContextPath()%>/admin/publicNumber/publicNumber/batchUpload" target="_blank">
                ...
                <div class="panel panel-default m-b-none">
                    <table class="table text-center table-bordered table-striped m-b-none">
                        ...
                        <tbody>
                        <c:if test="${publicNumbers.size() eq 0}">
                            <tr>
                                <td colspan="6">无数据</td>
                            </tr>
                        </c:if>
                        <c:forEach items="${publicNumbers}" var="publicNumber">
                            <tr>
                                <td>${publicNumber.id}</td>
                                <td>${publicNumber.name}</td>
                                <td>${publicNumber.categories[0].name}&nbsp;&nbsp;|&nbsp;&nbsp;${publicNumber.categories[1].name}</td>
                                <td>
                                    <c:if test="${publicNumber.recommend==1}">
                                        是
                                    </c:if>
                                    <c:if test="${publicNumber.recommend==0}">
                                        否
                                    </c:if>
                                </td>
                                <td>
                                    <c:if test="${publicNumber.hot==1}">
                                        是
                                    </c:if>
                                    <c:if test="${publicNumber.hot==0}">
                                        否
                                    </c:if>
                                </td>
                                <td>
                                    <a href="${publicNumber.qrCode}" target="_blank">
                                        <img src="${publicNumber.qrCode}" style="max-width: 350px;max-height: 80px"
                                             title="${publicNumber.qrCode}">
                                    </a>
                                </td>
                                <td>${publicNumber.createTime}</td>
                                <td>
                                    <sec:authorize
                                            access="hasAnyRole(${sessionScope.sec_op.publicNumber_publicNumber_get})">
                                        <button class="btn btn-warning btn-xs"
                                                onclick="detailsPublicNumber(${publicNumber.id})">
                                            详情
                                        </button>
                                    </sec:authorize>
                                    <sec:authorize
                                            access="hasAnyRole(${sessionScope.sec_op.publicNumber_publicNumber_update})">
                                        <button class="btn btn-info btn-xs"
                                                onclick="updatePublicNumber(${publicNumber.id})">
                                            编辑
                                        </button>
                                    </sec:authorize>
                                    <sec:authorize
                                            access="hasAnyRole(${sessionScope.sec_op.publicNumber_publicNumber_remove})">
                                        <button class="btn btn-danger btn-xs"
                                                onclick="remove(${publicNumber.id},'<%=request.getContextPath()%>/admin/publicNumber/publicNumber/remove')">
                                            删除
                                        </button>
                                    </sec:authorize>
                                </td>
...
<sec:authorize access="hasAnyRole(${sessionScope.sec_op.publicNumber_publicNumber_save})">
<%--新增--%>
    <div class="modal fade" id="savePublicNumber" data-backdrop="static" role="dialog">
    ...
    <script type="text/javascript">
        ...
        var saveValidate = $saveForm.validate({
            ...
            submitHandler:function () {
                doPost("<%=request.getContextPath()%>/admin/publicNumber/publicNumber/save", $saveForm.serialize(), function (data) {
                    if (data.status) {
...
//                        },800);
        ...
        $('#savePublicNumber').on('hidden.bs.modal', function (e) {
...
<sec:authorize access="hasAnyRole(${sessionScope.sec_op.publicNumber_publicNumber_get})">
    <%--公众号详情--%>
    <div class="modal fade" id="detailsPublicNumber" data-backdrop="static" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
    ...
    <script type="text/javascript">
        ...
        var detailsPublicNumberId;
        function detailsPublicNumber(id) {
            detailsPublicNumberId = id;
            //获取公众号信息
            doPost("<%=request.getContextPath()%>/admin/publicNumber/publicNumber/get", {id: detailsPublicNumberId}, function (data) {
                var _data = data.data;
                if (data.status) {
...
//                    $detailsForm.find("span[name='updateTime']").html(_data.updateTime);
            ...
            $("#detailsPublicNumber").modal("show");
        }
        function submitDetails() {
            $("#detailsPublicNumber").modal("hide");
        }
    </script>
    <%--公众号详情结束--%>
        </sec:authorize>
<sec:authorize access="hasAnyRole(${sessionScope.sec_op.publicNumber_publicNumber_update})">
    <%--编辑公众号开始--%>
    <div class="modal fade" id="updatePublicNumber" data-backdrop="static" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
    ...
    <script type="text/javascript">
        ...
        var updateValidate = $updateForm.validate({
            ...
            submitHandler:function () {
                doPost("<%=request.getContextPath()%>/admin/publicNumber/publicNumber/update", $updateForm.serialize(), function (data) {
                    if (data.status) {
                        $("#info_success").modal("show");
                        setTimeout(function(){
                            window.location.reload(true);
        ...
        function updatePublicNumber(id) {
            updatePublicNumberId = id;
            updateValidate.resetForm();
            $updateForm[0].reset();
            //获取公众号信息
            doPost("<%=request.getContextPath()%>/admin/publicNumber/publicNumber/get", {id: updatePublicNumberId}, function (data) {
                var _data = data.data
                if (data.status) {
...
//            这是需要修改图片路径，模块
    ...
    <script>
        ...
        var zTreeNodes = [];
        function asyncUrl() {
            return "<%=request.getContextPath()%>/admin/publicNumber/publicNumberCategory/tree";
        }
