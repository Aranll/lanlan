    ...
    <div class="app-content ">
        <div class="app-content-body">
            <div class="bg-light lter b-b wrapper-md ">
                <h1 class="m-n font-thin h3">图书分类</h1>
            </div>
            <div class="wrapper-md">
                <form class="form-horizontal" id="searchForm">
                    ...
                    <div class="form-group m-t-n-md">
                        <div class="col-xs-12">
                            <sec:authorize access="hasAnyRole(${sessionScope.sec_op.book_category_save})">
                    ...
                    <table class="table text-center table-bordered table-striped m-b-none">
                        ...
                        <c:forEach items="${books}" var="book">
                            <tr>
                                <td>${book.id}</td>
                                <td>${book.name}</td>
                                <td>${book.parent.name}</td>
                                <td>
                                    <c:if test="${book.hot==1}">
                                        是
                                    </c:if>
                                    <c:if test="${book.hot==0}">
                                        否
                                    </c:if>
                                </td>
                                <td>${book.statusDesc}</td>
                                <td>
                                    <sec:authorize access="hasAnyRole(${sessionScope.sec_op.book_category_save})">
                                    <button class="btn btn-warning btn-xs" onclick="detailsBook(${book.id})">
                                        详情
                                    </button>
                                    </sec:authorize>
                                    <sec:authorize access="hasAnyRole(${sessionScope.sec_op.book_category_save})">
                                    <button class="btn btn-info btn-xs" onclick="updateBook(${book.id})">
                                        编辑
                                    </button>
                                    </sec:authorize>
                                    <sec:authorize access="hasAnyRole(${sessionScope.sec_op.book_category_save})">
                                    <button class="btn btn-danger btn-xs" onclick="remove(${book.id},'<%=request.getContextPath()%>/admin/book/bookCategory/remove')">
                                        删除
                                    </button>
                                    </sec:authorize>
                                </td>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">新增图书分类</h4>
                </div>
...
//                digits: "所属是个编号"
        ...
        //使用ajax提交表单，保存图书分类
        function submitSave() {
            doPost("<%=request.getContextPath()%>/admin/book/bookCategory/save", $saveForm.serialize(), function (data) {
                if (data.status) {
...
//                    },600);
    ...
    <%--图书详情--%>
    <div class="modal fade" id="detailsBook" data-backdrop="static" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
    ...
    <script type="text/javascript">
        ...
        function detailsBook(id) {
            detailsBookId = id;
            //获取图书信息
            doPost("<%=request.getContextPath()%>/admin/book/bookCategory/get", {id: detailsBookId}, function (data) {
                var _data = data.data;
                if (data.status) {
        ...
        function submitDetails() {
            $("#detailsBook").modal("hide");
        }
    </script>
    <%--图书详情结束--%>

    <%--编辑图书开始--%>
    <div class="modal fade" id="updateBook" data-backdrop="static" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
    ...
    <script type="text/javascript">
        ...
        function updateBook(id) {
            ...
            $updateForm[0].reset();
            //获取图书信息
            doPost("<%=request.getContextPath()%>/admin/book/bookCategory/get", {id: updateBookId}, function (data) {
                var _data = data.data;
                if (data.status) {
        ...
        function submitUpdate() {
            doPost("<%=request.getContextPath()%>/admin/book/bookCategory/update", $updateForm.serialize(), function (data) {
                if (data.status) {
                    $("#info_success").modal("show");
                    setTimeout(function () {
                        window.location.reload(true);
    ...
    <script>
        ...
        var zTreeNodes = [];
        function asyncUrl() {
            return "<%=request.getContextPath()%>/admin/book/bookCategory/tree";
        }
