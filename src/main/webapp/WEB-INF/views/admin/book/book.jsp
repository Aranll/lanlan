            ...
            <div class="wrapper-md">
                <form class="form-horizontal" id="searchForm">
                    <div class="form-group">
                        ...
                        <div class="col-xs-8 col-md-4 col-lg-3 m-b-md">
                            <input type="text" onclick="showMenu(this)" name="category.name" class="form-control" value="${category.name}">
                    ...
                    <div class="form-group m-t-n-md">
                        <div class="col-xs-12">
                            <sec:authorize access="hasAnyRole(${sessionScope.sec_op.book_book_save})">
                            <a href="<%=request.getContextPath()%>/admin/book/book/new?redirectUrl=${redirectUrl}">
                                <button class="btn btn-success" type="button" data-toggle="modal"
                                        data-target="#savebook">
                                    新增
                                </button>
                            </a>
                            </sec:authorize>
                            <sec:authorize access="hasAnyRole(${sessionScope.sec_op.book_book_batchUpload})">
                            <a href="<%=request.getContextPath()%>/admin/book/book/book/batchUpload" target="_blank">
                    ...
                    <table class="table text-center table-bordered table-striped m-b-none">
                        ...
                        <c:forEach items="${books}" var="book">
                            <tr>
                                <td>${book.id}</td>
                                <td>${book.name}</td>
                                <td>${book.categories[0].name}&nbsp;&nbsp;|&nbsp;&nbsp;${book.categories[1].name}</td>
                                <td>
                                ...
                                <td>
                                    <c:if test="${book.accessVipLevel==1}">
                                        是
                                    </c:if>
                                    <c:if test="${book.accessVipLevel==0}">
                                        否
                                    </c:if>
                                </td>
                                <td><a href="${book.cover}" target="_blank">
                                    <img src="${book.cover}"
                                         style="max-width: 350px;max-height: 80px" title="${book.cover}">
                                </a></td>
                                <td>${book.createTime}</td>
                                <td>
                                    <sec:authorize access="hasAnyRole(${sessionScope.sec_op.book_book_get})">
                                    <a href="<%=request.getContextPath()%>/admin/book/book/get?id=${book.id}&redirectUrl=${redirectUrl}">
                                    ...
                                    <a href="<%=request.getContextPath()%>/admin/book/book/edit?id=${book.id}&redirectUrl=${redirectUrl}">
                                        <button class="btn btn-info btn-xs">
                                            编辑
                                        </button>
                                    </a>
                                    </sec:authorize>
                                    <sec:authorize access="hasAnyRole(${sessionScope.sec_op.book_book_remove})">
                                    <button class="btn btn-danger btn-xs" onclick="remove(${book.id},'<%=request.getContextPath()%>/admin/book/book/remove')">
                                        删除
                                    </button>
                                    </sec:authorize>
                                </td>
