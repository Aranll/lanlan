    ...
    <div class="app-content ">
        <div class="app-content-body">
            <div class="bg-light lter b-b wrapper-md ">
                <h1 class="m-n font-thin h3">APP分类</h1>
            </div>
            <div class="wrapper-md">
                <form class="form-horizontal" id="searchForm">
                    ...
                    <div class="form-group m-t-n-md">
                        <div class="col-xs-12">
                            <sec:authorize access="hasAnyRole(${sessionScope.sec_op.app_category_save})">
                    ...
                    <table class="table text-center table-bordered table-striped m-b-none">
                        ...
                        <c:forEach items="${apps}" var="app">
                            <tr>
                                ...
                                <td>${app.statusDesc}</td>
                                <td>
                                    <sec:authorize access="hasAnyRole(${sessionScope.sec_op.app_category_get})">
                                    <button class="btn btn-warning btn-xs" onclick="detailsApp(${app.id})">
                                        详情
                                    </button>
                                    </sec:authorize>
                                    <sec:authorize access="hasAnyRole(${sessionScope.sec_op.app_category_update})">
                                    <button class="btn btn-info btn-xs" onclick="updateApp(${app.id})">
                                        编辑
                                    </button>
                                    </sec:authorize>
                                    <sec:authorize access="hasAnyRole(${sessionScope.sec_op.app_category_remove})">
                                    <button class="btn btn-danger btn-xs" onclick="remove(${app.id},'<%=request.getContextPath()%>/admin/app/appCategory/remove')">
                                        删除
                                    </button>
                                    </sec:authorize>
                                </td>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">新增APP分类</h4>
                </div>
...
//                digits: "所属是个编号"
        ...
        //使用ajax提交表单，保存APP分类
        function submitSave() {
            doPost("<%=request.getContextPath()%>/admin/app/appCategory/save", $saveForm.serialize(), function (data) {
                if (data.status) {
...
<sec:authorize access="hasAnyRole(${sessionScope.sec_op.app_category_get})">
    ...
    <script type="text/javascript">
        ...
        function detailsApp(id) {
            detailsAppId = id;
            //获取APP信息
            doPost("<%=request.getContextPath()%>/admin/app/appCategory/get", {id: detailsAppId}, function (data) {
                console.log(data);
                var _data = data.data;
                console.log(_data);
                if (data.status) {
...
<%--编辑APP开始--%>
    ...
    <script type="text/javascript">
        ...
        function updateApp(id) {
            ...
            $updateForm[0].reset();
            //获取APP信息
            doPost("<%=request.getContextPath()%>/admin/app/appCategory/get", {id: updateAppId}, function (data) {
                console.log(data);
                var _data = data.data;
                console.log(_data);
                if (data.status) {
        ...
        function submitUpdate() {
            doPost("<%=request.getContextPath()%>/admin/app/appCategory/update", $updateForm.serialize(), function (data) {
                if (data.status) {
                    $("#info_success").modal("show");
                    setTimeout(function () {
                        window.location.reload(true);
    ...
    <script>
        ...
        var zTreeNodes = [];
        function asyncUrl() {
            return "<%=request.getContextPath()%>/admin/app/appCategory/tree";
        }
