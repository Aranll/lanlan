...
<div class="app app-header-fixed app-aside-fixed">
    ...
    <%@include file="./miniApp_nav.jsp" %>
    <div class="app-content ">
        <div class="app-content-body">
            <div class="bg-light lter b-b wrapper-md ">
                <h1 class="m-n font-thin h3">小程序管理</h1>
            </div>
            <div class="wrapper-md">
                <form class="form-horizontal" id="searchForm">
                    <div class="form-group">
                        ...
                        <div class="col-xs-8 col-md-4 col-lg-3 m-b-md">
                            <input type="text" onclick="showMenu(this,0)" name="category.name" class="form-control" value="${category.name}">
                    ...
                    <div class="form-group m-t-n-md">
                        <div class="col-xs-12">
                            <sec:authorize access="hasAnyRole(${sessionScope.sec_op.miniApp_miniApp_save})">
                                <button class="btn btn-success" type="button" data-toggle="modal"
                                        data-target="#saveMiniApp">
                                    新增
                                </button>
                            </sec:authorize>
                            <sec:authorize access="hasAnyRole(${sessionScope.sec_op.miniApp_miniApp_batchUpload})">
                            <a href="<%=request.getContextPath()%>/admin/miniApp/miniApp/batchUpload">
                ...
                <div class="panel panel-default m-b-none">
                    <table class="table text-center table-bordered table-striped m-b-none">
                        ...
                        <tbody>
                        <c:if test="${miniApps.size() eq 0}">
                            <tr>
                                <td colspan="6">无数据</td>
                            </tr>
                        </c:if>
                        <c:forEach items="${miniApps}" var="miniApp">
                            <tr>
                                <td>${miniApp.id}</td>
                                <td>${miniApp.name}</td>
                                <td>${miniApp.categories[0].name}&nbsp;&nbsp;|&nbsp;&nbsp;${miniApp.categories[1].name}</td>
                                <td>
                                    <c:if test="${miniApp.recommend==1}">
                                        是
                                    </c:if>
                                    <c:if test="${miniApp.recommend==0}">
                                        否
                                    </c:if>
                                </td>
                                <td>
                                    <c:if test="${miniApp.hot==1}">
                                        是
                                    </c:if>
                                    <c:if test="${miniApp.hot==0}">
                                        否
                                    </c:if>
                                </td>
                                <td>
                                    <a href="${miniApp.qrCode}" target="_blank">
                                        <img src="${miniApp.qrCode}" style="max-width: 350px;max-height: 80px"
                                             title="${miniApp.qrCode}">
                                    </a>
                                </td>
                                <td>${miniApp.createTime}</td>
                                <td>
                                    <sec:authorize access="hasAnyRole(${sessionScope.sec_op.miniApp_miniApp_get})">
                                        <button class="btn btn-warning btn-xs" onclick="detailsMiniApp(${miniApp.id})">
                                            详情
                                        </button>
                                    </sec:authorize>
                                    <sec:authorize access="hasAnyRole(${sessionScope.sec_op.miniApp_miniApp_update})">
                                        <button class="btn btn-info btn-xs" onclick="updateMiniApp(${miniApp.id})">
                                            编辑
                                        </button>
                                    </sec:authorize>
                                    <sec:authorize access="hasAnyRole(${sessionScope.sec_op.miniApp_miniApp_remove})">
                                        <button class="btn btn-danger btn-xs"
                                                onclick="remove(${miniApp.id},'<%=request.getContextPath()%>/admin/miniApp/miniApp/remove')">
                                            删除
                                        </button>
                                    </sec:authorize>
                                </td>
...
<sec:authorize access="hasAnyRole(${sessionScope.sec_op.miniApp_miniApp_save})">
<%--新增分类--%>
    <div class="modal fade" id="saveMiniApp" data-backdrop="static" role="dialog">
    ...
    <script type="text/javascript">
        ...
        var saveValidate = $saveForm.validate({
            ...
            submitHandler:function () {
                doPost("<%=request.getContextPath()%>/admin/miniApp/miniApp/save", $saveForm.serialize(), function (data) {
                    if (data.status) {
...
//                        },600);
        ...
        $('#saveMiniApp').on('hidden.bs.modal', function (e) {
...
<sec:authorize access="hasAnyRole(${sessionScope.sec_op.miniApp_miniApp_get})">
<%--小程序详情--%>
    <div class="modal fade" id="detailsMiniApp" data-backdrop="static" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
    ...
    <script type="text/javascript">
        ...
        var detailsMiniAppId;
        function detailsMiniApp(id) {
            detailsMiniAppId = id;
            //获取小程序信息
            doPost("<%=request.getContextPath()%>/admin/miniApp/miniApp/get", {id: detailsMiniAppId}, function (data) {
                console.log(data);
                var _data = data.data;
                console.log(_data);
                if (data.status) {
            ...
            $("#detailsMiniApp").modal("show");
        }
        function submitDetails() {
            $("#detailsMiniApp").modal("hide");
        }
    </script>
    <%--小程序详情结束--%>
        </sec:authorize>
<sec:authorize access="hasAnyRole(${sessionScope.sec_op.miniApp_miniApp_update})">
<%--编辑小程序开始--%>
    <div class="modal fade" id="updateMiniApp" data-backdrop="static" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
    ...
    <script type="text/javascript">
        ...
        var updateValidate = $updateForm.validate({
            ...
            submitHandler:function () {
                doPost("<%=request.getContextPath()%>/admin/miniApp/miniApp/update", $updateForm.serialize(), function (data) {
                    if (data.status) {
                        $("#info_success").modal("show");
        ...
        function updateMiniApp(id) {
            updateMiniAppId = id;
            updateValidate.resetForm();
            $updateForm[0].reset();
            //获取小程序信息
            doPost("<%=request.getContextPath()%>/admin/miniApp/miniApp/get", {id: updateMiniAppId}, function (data) {
                var _data = data.data
                if (data.status) {
...
//            这是需要修改图片路径，模块
    ...
    <script>
        ...
        var zTreeNodes = [];
        function asyncUrl() {
            return "<%=request.getContextPath()%>/admin/miniApp/miniApp/tree";
        }
