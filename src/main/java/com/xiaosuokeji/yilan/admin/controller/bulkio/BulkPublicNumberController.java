...
/**
 * 批量操作程序<br/>
 * Created by ljr on 2017/8/22.
 */
@Controller("bulkPublicNumberController")
@RequestMapping("/admin")
public class BulkPublicNumberController {

    @RequestMapping(value = "/publicNumber/template/download", method = RequestMethod.POST)
    public void downloadTemplate(String[] qrCodes, HttpServletRequest request, HttpServletResponse response) throws Exception {
        JSONObject jsonObject = WebUtils.doRawRequest(API.PUBLICNUMBER_CATEGORY_TREE, new PublicNumber());
        ...
        response.setHeader("Content-disposition", "attachment; filename=" +
                URLEncoder.encode("医览网-公众号模板.xlsx", "UTF-8"));
        wb.write(response.getOutputStream());
    }

    @RequestMapping(value = "/publicNumber/template/upload", method = RequestMethod.POST)
    @ResponseBody
    public String uploadTemplate(MultipartFile file) throws Exception {
        Workbook wb = WorkbookFactory.create(file.getInputStream());
        List<PublicNumber> publicNumbers = new ArrayList<>();
        for (int i=0; i<wb.getNumberOfSheets(); ++i) {
            Sheet sheet = wb.getSheetAt(i);
            for (int j=1; j<=sheet.getLastRowNum(); ++j) {
                Row row = sheet.getRow(j);
                PublicNumber publicNumber = new PublicNumber();
                publicNumber.setName(row.getCell(0) == null ? "" : row.getCell(0).getStringCellValue());
                publicNumber.setQrCode(row.getCell(1) == null ? "" : row.getCell(1).getStringCellValue());
                publicNumber.setDeveloper(row.getCell(2) == null ? "" : row.getCell(2).getStringCellValue());
        ...
        PublicNumber publicNumber =new PublicNumber();
        publicNumber.setPublicNumbers(publicNumbers);
        return WebUtils.doRawRequest(API.PUBLICNUMBER_MULTI_SAVE, publicNumber).toString();
    }
}
