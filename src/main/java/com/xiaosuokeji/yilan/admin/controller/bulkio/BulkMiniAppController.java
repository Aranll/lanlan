...
@Controller("bulkMiniAppController")
@RequestMapping("/admin")
public class BulkMiniAppController {

    @RequestMapping(value = "/miniApp/template/download", method = RequestMethod.POST)
    public void downloadTemplate(String[] qrCodes, HttpServletRequest request, HttpServletResponse response) throws Exception {
        JSONObject jsonObject = WebUtils.doRawRequest(API.MINIAPP_CATEGORY_TREE, new MiniApp());
        ...
        response.setHeader("Content-disposition", "attachment; filename=" +
                URLEncoder.encode("医览网-小程序模板.xlsx", "UTF-8"));
        wb.write(response.getOutputStream());
    }

    @RequestMapping(value = "/miniApp/template/upload", method = RequestMethod.POST)
    @ResponseBody
    public String uploadTemplate(MultipartFile file) throws Exception {
        Workbook wb = WorkbookFactory.create(file.getInputStream());
        List<MiniApp> miniApps = new ArrayList<>();
        for (int i=0; i<wb.getNumberOfSheets(); ++i) {
            Sheet sheet = wb.getSheetAt(i);
            for (int j=1; j<=sheet.getLastRowNum(); ++j) {
                Row row = sheet.getRow(j);
                MiniApp miniApp = new MiniApp();
                miniApp.setName(row.getCell(0) == null ? "" : row.getCell(0).getStringCellValue());
                miniApp.setQrCode(row.getCell(1) == null ? "" : row.getCell(1).getStringCellValue());
                miniApp.setDeveloper(row.getCell(2) == null ? "" : row.getCell(2).getStringCellValue());
        ...
        MiniApp miniApp = new MiniApp();
        miniApp.setMiniApps(miniApps);
        return WebUtils.doRawRequest(API.BULK_MINIAPP_IMPORT, miniApp).toString();
    }
}
